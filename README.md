...
Empty days continue the current location. Dates, coordinates, times, costs, bookings and preparation lists are optional; unknown information stays unknown. **Area** filters countries across Overview, Map, Calendar and Bookings; a country becomes an Area once you spend a night there, so home airports and layovers stay on the map without their own Area. Checklist always covers the whole trip, so Area stays locked on it. When a trip visits more than one country, its whole-trip map is a globe: drag to turn it and scroll to zoom. A single country stays a flat map. A day's card shows its must-know note, the sights to see with an optional tip and Google Maps link, the route, and its documents. Once a trip has a booking, the Calendar marks nights and flights that aren't booked yet in amber, with a count above the grid. The map and calendar share a timeline; map lines and untimed animation are schematic.
