...
- Trip options: `title`, `startDate` (real `YYYY-MM-DD`), `initialPlace` (a place ID), optional `groups` (described below), `documents`, `bookings`, `prepare` (described below), and `timezone` (IANA fallback for places without their own zone). Without a start date, days have Day 1, Day 2 labels.
