...
// `--mode pages` builds the public demo site for GitHub Pages at /trip-atlas/.
export default defineConfig(({ mode }) => {
  const outDir = mode === "pages" ? "dist-pages" : "dist";
  return {
    base: mode === "pages" ? "/trip-atlas/" : "/",
    build: { outDir },
    plugins: [
      react(),
      ...
      license({
        thirdParty: {
          ...
          output: {
            file: fileURLToPath(
              new URL(`./${outDir}/THIRD_PARTY_LICENSES.txt`, import.meta.url),
            ),
